...
<div class="container-fluid px-2 px-md-4 mb-5">
    <div class="bg-white border-0 shadow-sm rounded-4 p-3 p-md-4 mb-4 mt-2">
        ...
        <h4 class="fw-bold text-dark m-0">Sửa vị trí kệ</h4>
        <div class="text-muted small">Cập nhật tên vị trí lưu trữ trong kho</div>
    </div>
        ...
        <div class="card-body p-3 p-md-4">
            <form action="{{ route('locations.update', $location->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="shelf_name" class="form-control" value="{{ old('shelf_name', $location->shelf_name) }}" required>
                    @error('shelf_name')<small class="text-danger">{{ $message }}</small>@enderror
                </div>
                <button type="submit" class="btn btn-primary fw-bold">
                    <i class="bi bi-save me-1"></i>Cập nhật vị trí
                </button>
            </form>
        </div>
    </div>
