...
<div class="container-fluid px-2 px-md-4 mb-5">
    <div class="bg-white border-0 shadow-sm rounded-4 p-3 p-md-4 mb-3">
        ...
        <div class="text-uppercase text-primary fw-bold small mb-1">Phiếu nhập</div>
    ...
    <div class="row g-2 g-md-3 mb-3">
        ...
        <div class="col-6 col-lg-3"><div class="bg-white shadow-sm rounded-4 p-3 h-100"><div class="text-muted small fw-bold">Người nhập</div><div class="fw-bold text-dark">{{ $importVoucher->user?->displayName() ?: 'Hệ thống' }}</div></div></div>
        ...
        @if($canViewCost)
            <div class="col-6 col-lg-3"><div class="bg-white shadow-sm rounded-4 p-3 h-100"><div class="text-muted small fw-bold">Tổng giá vốn</div><div class="fw-bold text-dark">{{ number_format($importVoucher->total_cost) }} đ</div></div></div>
        @endif
    </div>
                ...
                <div class="d-flex flex-column flex-md-row justify-content-between gap-2 mb-3">
                    ...
                    @if($canViewCost)
                        <div class="text-md-end">
                            <div class="small text-muted">Đơn giá vốn</div>
                            <div class="fw-bold">{{ number_format($item->unit_cost) }} đ</div>
                            <div class="small text-muted">Tổng {{ number_format($item->total_cost) }} đ</div>
                        </div>
                    @endif
                </div>
                        ...
                        <tbody>
                            @forelse($item->products as $product)
                                <tr>
                                    ...
                                    <td><span class="badge {{ $product->status == 1 ? 'bg-success' : 'bg-primary' }}">{{ $product->status == 1 ? 'Còn trong kho' : 'Đã xuất' }}</span></td>
                            ...
                            @empty
                                <tr><td colspan="4" class="text-center text-muted py-4">Chưa có serial.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
    ...
    @empty
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-body text-center text-muted py-4">Chưa có dòng hàng.</div>
        </div>
    @endforelse
</div>
@endsection
