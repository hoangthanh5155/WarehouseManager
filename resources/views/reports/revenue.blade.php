...
<div class="container-fluid px-2 px-md-4 mb-5">
    <div class="bg-white border-0 shadow-sm rounded-4 p-3 p-md-4 mb-3">
        ...
        <h3 class="fw-bold text-dark m-0">Doanh thu</h3>
        ...
        <div class="d-flex gap-2 flex-wrap mt-3">
            ...
            @endforeach
            <a href="{{ route('reports.revenue', ['period' => 'custom', 'start_date' => $startDate->toDateString(), 'end_date' => $endDate->toDateString()]) }}" class="btn btn-sm {{ $period === 'custom' ? 'btn-primary' : 'btn-outline-primary' }} fw-bold">Tùy chọn</a>
        </div>
        ...
        <form action="{{ route('reports.revenue') }}" method="GET" class="row g-2 align-items-end mt-3 {{ $period === 'custom' ? '' : 'd-none' }}">
            ...
            <div class="col-12 col-md-auto">
                <button type="submit" class="btn btn-primary fw-bold px-4 w-100"><i class="bi bi-funnel me-1"></i>Lọc</button>
            </div>
        </form>
    </div>
    ...
    <div class="row g-2 g-md-3 mb-3">
        ...
        @if($canViewCost)
            <div class="col-6 col-lg-3"><div class="bg-white shadow-sm rounded-4 p-3 h-100"><div class="text-muted small fw-bold">Giá vốn</div><div class="fs-4 fw-bold">{{ $formatMoney($totalCost) }}</div></div></div>
            <div class="col-6 col-lg-3"><div class="bg-white shadow-sm rounded-4 p-3 h-100"><div class="text-muted small fw-bold">Lợi nhuận gộp</div><div class="fs-4 fw-bold {{ $grossProfit >= 0 ? 'text-success' : 'text-danger' }}">{{ $formatMoney($grossProfit) }}</div></div></div>
        @endif
        <div class="col-6 col-lg-3"><div class="bg-white shadow-sm rounded-4 p-3 h-100"><div class="text-muted small fw-bold">Phiếu xuất</div><div class="fs-4 fw-bold">{{ number_format($exportOrderCount) }}</div></div></div>
        <div class="col-6 col-lg-3"><div class="bg-white shadow-sm rounded-4 p-3 h-100"><div class="text-muted small fw-bold">Serial đã xuất</div><div class="fs-4 fw-bold">{{ number_format($exportedSerialCount ?: $exportedProductCount) }}</div></div></div>
    </div>

    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-body p-3">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h5 class="fw-bold text-dark m-0">Phiếu xuất</h5>
                    ...
                    <tbody>
                        @forelse($vouchers as $voucher)
                            ...
                            <tr>
                                ...
                                <td><span class="badge {{ $voucher->customer_type === 'agency' ? 'bg-success' : 'bg-danger' }}">{{ $voucher->customer_type === 'agency' ? 'Đại lý' : 'Khách lẻ' }}</span></td>
                                ...
                                <td class="text-end">
                                    <a href="{{ route('export.print', $voucher->id) }}" class="btn btn-sm btn-outline-primary fw-bold"><i class="bi bi-eye me-1"></i>Xem hóa đơn</a>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="{{ $canViewCost ? 9 : 7 }}" class="text-center text-muted py-4">Chưa có dữ liệu.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
