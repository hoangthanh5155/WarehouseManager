...
@else
    <form method="POST" action="{{ route('shop.cart.update') }}" class="card border-0 shadow-sm">
        @csrf
        <div class="table-responsive">
            <table class="table align-middle mb-0">
                <thead class="table-light"><tr><th>Sản phẩm</th><th class="text-end">Giá</th><th style="width:140px;">SL</th><th class="text-end">Tổng</th><th></th></tr></thead>
                <tbody>
                    @foreach($items as $item)
                        <tr>
                            <td class="fw-semibold">{{ $item['catalog']->product_name }}</td>
                            <td class="text-end">{{ number_format($item['unit_price']) }} đ</td>
                            <td><input class="form-control" type="number" min="0" name="items[{{ $item['catalog']->id }}]" value="{{ $item['quantity'] }}"></td>
